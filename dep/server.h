...
public:
    ...
    void get(std::string dyn_route_str, const std::function< void(const crow::request&, crow::response& res) >& cb){
    ...
    void post(std::string dyn_route_str, const std::function< void(const crow::request&, crow::response& res) >& cb){
        
        char dyn_route[64];
        strncpy(dyn_route, dyn_route_str.c_str(), sizeof(dyn_route));
        dyn_route[sizeof(dyn_route) - 1] = 0;

        route_dynamic(dyn_route).methods("POST"_method)(cb);
    }
